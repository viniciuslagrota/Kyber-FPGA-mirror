...
//	memcpy(r, (poly *)memoryBram0, 1024);
	...
	memcpy(TxBufferPtr, (u8*)r, 1024);

	Xil_DCacheFlushRange((UINTPTR)TxBufferPtr, 1024);

	//Configure RX
	XAxiDma_SimpleTransfer(&XAxiDmaPtr,(UINTPTR) RxBufferPtr, 1024, XAXIDMA_DEVICE_TO_DMA);

	//Configure TX
	XAxiDma_SimpleTransfer(&XAxiDmaPtr,(UINTPTR) TxBufferPtr, 1024, XAXIDMA_DMA_TO_DEVICE);
	...
	XGpio_DiscreteWrite(&XGpioDma, 2, 256); //Number of words, not bytes.
	...
	memcpy(r, (polyvec *)RxBufferPtr, 1024);
...
//	memcpy(r, (poly *)memoryBram1, 512);
...
#if  KYBER_K == 2
	memcpy(TxBufferPtr, (u8*)a, 1024);
	memcpy(TxBufferPtr + 1024, (u8*)b, 1024);
#elif KYBER_K == 3
	memcpy(TxBufferPtr, (u8*)a, 1536);
	memcpy(TxBufferPtr + 1536, (u8*)b, 1536);
#else
	memcpy(TxBufferPtr, (u8*)a, 2048);
	memcpy(TxBufferPtr + 2048, (u8*)b, 2048);
#endif

	Xil_DCacheFlushRange((UINTPTR)TxBufferPtr, 2048);

	//Configure RX
	XAxiDma_SimpleTransfer(&XAxiDmaPtr,(UINTPTR) RxBufferPtr, 2048, XAXIDMA_DEVICE_TO_DMA);

	//Configure TX
	XAxiDma_SimpleTransfer(&XAxiDmaPtr,(UINTPTR) TxBufferPtr, 2048, XAXIDMA_DMA_TO_DEVICE);
	...
	//Send length to be read
	XGpio_DiscreteWrite(&XGpioDma, 2, 512); //Number of words, not bytes.
	...
	memcpy(r, (poly *)RxBufferPtr, 2048);
