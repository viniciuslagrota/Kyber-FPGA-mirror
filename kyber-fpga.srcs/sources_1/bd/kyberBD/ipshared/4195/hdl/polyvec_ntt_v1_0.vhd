...
architecture arch_imp of polyvec_ntt_v1_0 is
    ...
    -- INDEXERS NTT
    constant index_ntt_max                  : integer := 10;
...
--                s_bram_addrb <= std_logic_vector(unsigned(s_last_addra) + 1);
                ...
                if(s_start = '1') then                       
                    ...
                    if(s_kyber_k_it(9) < unsigned(s_kyber_k)) then
                        if(s_en_vec(8) = '1') then
                            s_bram_wea <= '1';  
                            s_bram_addra <= "0" & std_logic_vector(s_kyber_k_it(10)) & std_logic_vector(s_j(9) + s_length(9));
                            s_bram_dia   <= std_logic_vector(unsigned(s_bram_dob_vec(5)(31 downto 16)) - unsigned(s_coeff_from_fqmul1))
                                          & std_logic_vector(unsigned(s_bram_dob_vec(5)(15 downto 0)) - unsigned(s_coeff_from_fqmul0));
                        else
                            s_bram_wea <= '0';                                    
                ...
                if(s_start = '1') then                       
                    ...
                    if(s_kyber_k_it(9) < unsigned(s_kyber_k)) then
                        if(s_en_vec(8) = '1') then
                            s_bram_web <= '1';  
                            s_bram_addrb <= "0" & std_logic_vector(s_kyber_k_it(10)) & std_logic_vector(s_j(9));
                            s_bram_dib   <= std_logic_vector(unsigned(s_bram_dob_vec(5)(31 downto 16)) + unsigned(s_coeff_from_fqmul1))
                                          & std_logic_vector(unsigned(s_bram_dob_vec(5)(15 downto 0)) + unsigned(s_coeff_from_fqmul0));
                        else
                            s_bram_web   <= '0';         
