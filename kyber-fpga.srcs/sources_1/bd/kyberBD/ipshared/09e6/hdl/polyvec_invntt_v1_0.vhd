...
architecture arch_imp of polyvec_invntt_v1_0 is
    ...
    -- SIGNAL FROM BARRETT
    constant barrett_len                 : integer := 2;
    ...
    -- INDEXERS NTT
    constant index_ntt_max                  : integer := 11;
    ...
    constant fqmul_en_len                   : integer := 3;
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                -- Se colocar registros no fqmul, é aqui que deve ser adaptado. Provavelmente não precisará de delays no vetor
                -- ou talvez apenas um, para entrar no pulso de escrita. O registro de entrada do barrett que deve ter que ser vetorizado,
                -- já que ele deve ficar mais lento que o fqmul, caso registre mais de uma vez o fqmul.
...
--                s_bram_addrb <= std_logic_vector(unsigned(s_last_addra) + 1);
                ...
                if(s_start = '1') then   
                    s_bram_ena   <= s_start; 
                    if(s_kyber_k_it(11) < unsigned(s_kyber_k)) then
                        if(s_valid0_from_barrett(1) = '1') then
                            s_bram_wea <= '1';                                    
                            s_bram_addra <= "0" & std_logic_vector(s_kyber_k_it(11)) & std_logic_vector(s_j(11) + s_length(11));
                            s_bram_dia   <= s_coeff_from_fqmul1(2) & s_coeff_from_fqmul0(2);
                        else                          
                            s_bram_wea <= '0';                                    
                            s_bram_addra <= "0" & std_logic_vector(s_kyber_k_it(1)) & std_logic_vector(s_j(0) + s_length(0));
                            s_bram_dia   <= (others => '0');
                        end if;
                    elsif((s_kyber_k_it(11) = unsigned(s_kyber_k)) and (s_kyber_k_it(12) = unsigned(s_kyber_k) - 1)) then   
                        s_bram_wea   <= '0';
                        s_bram_addra <= (others => '0');
                        s_bram_dia   <= (others => '0');
                    elsif((s_kyber_k_it(13) = unsigned(s_kyber_k))) then  
                        s_bram_wea <= '0';   
                ...
                if(s_start = '1') then                       
                    s_bram_enb   <= s_start; 
                    if(s_kyber_k_it(11) < unsigned(s_kyber_k)) then
                        if(s_valid1_from_barrett(1) = '1') then
                            s_bram_web   <= '1';         
                            s_bram_addrb <= "0" & std_logic_vector(s_kyber_k_it(11)) & std_logic_vector(s_j(11));               
                            s_bram_dib   <= s_data1_from_barrett(1) & s_data0_from_barrett(1);
                        else
                            s_bram_web   <= '0';         
                        ...
                        s_bram_web <= s_valid_from_fqmul0;   
                        s_bram_dib <= s_coeff_from_fqmul1(0) & s_coeff_from_fqmul0(0);
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then  
                ...
                s_data0_to_barrett <= std_logic_vector(unsigned(s_bram_doa(15 downto 0)) + unsigned(s_bram_dob(15 downto 0)));
                s_data1_to_barrett <= std_logic_vector(unsigned(s_bram_doa(31 downto 16)) + unsigned(s_bram_dob(31 downto 16)));
            end if;
        end if;
    end process;   
            ...
            if aresetn = '0' then
                ...
                if(s_start = '1') then
                    ...
                    if((s_kyber_k_it(11) < unsigned(s_kyber_k))) then  
                        ...
                        s_coeff1_to_fqmul0 <= std_logic_vector(signed(s_bram_doa(15 downto 0)) - signed(s_bram_dob(15 downto 0)));
                        -- fqmul1
                        s_coeff0_to_fqmul1 <= zetas(to_integer(s_idx_zeta(3)));
                        s_coeff1_to_fqmul1 <= std_logic_vector(signed(s_bram_doa(31 downto 16)) - signed(s_bram_dob(31 downto 16)));
                    else
                        -- valids
                        s_valid_to_fqmul0 <= s_en_fqmul_vec(1);
                        s_valid_to_fqmul1 <= s_en_fqmul_vec(1);  
                        -- fqmul0
