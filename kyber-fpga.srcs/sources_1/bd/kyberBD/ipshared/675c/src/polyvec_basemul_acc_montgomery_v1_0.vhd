...
--    signal s_bram_read_dib           : std_logic_vector(DATA_WIDTH-1 downto 0); 
    ...
    signal s_en_vec                  : std_logic_vector(16 downto 0);
...
--    signal zetas                     : zeta_type := (0 => x"fbec", 1 => x"fd0a", 2 => x"fe99", 3 => x"fa13", 4 => x"05d5", 5 => x"058e", 6 => x"011f", 7 => x"00ca", 8 => x"ff55", 9 => x"026e", 10 => x"0629", 11 => x"00b6", 12 => x"03c2", 13 => x"fb4e", 14 => x"fa3e", 15 => x"05bc", 16 => x"023d", 17 => x"fad3", 18 => x"0108", 19 => x"017f", 20 => x"fcc3", 21 => x"05b2", 22 => x"f9be", 23 => x"ff7e", 24 => x"fd57", 25 => x"03f9", 26 => x"02dc", 27 => x"0260", 28 => x"f9fa", 29 => x"019b", 30 => x"ff33", 31 => x"f9dd", 32 => x"04c7", 33 => x"028c", 34 => x"fdd8", 35 => x"03f7", 36 => x"faf3", 37 => x"05d3", 38 => x"fee6", 39 => x"f9f8", 40 => x"0204", 41 => x"fff8", 42 => x"fec0", 43 => x"fd66", 44 => x"f9ae", 45 => x"fb76", 46 => x"007e", 47 => x"05bd", 48 => x"fcab", 49 => x"ffa6", 50 => x"fef1", 51 => x"033e", 52 => x"006b", 53 => x"fa73", 54 => x"ff09", 55 => x"fc49", 56 => x"fe72", 57 => x"03c1", 58 => x"fa1c", 59 => x"fd2b", 60 => x"01c0", 61 => x"fbd7", 62 => x"02a5", 63 => x"fb05", 64 => x"fbb1", 65 => x"01ae", 66 => x"022b", 67 => x"034b", 68 => x"fb1d", 69 => x"0367", 70 => x"060e", 71 => x"0069", 72 => x"01a6", 73 => x"024b", 74 => x"00b1", 75 => x"ff15", 76 => x"fedd", 77 => x"fe34", 78 => x"0626", 79 => x"0675", 80 => x"ff0a", 81 => x"030a", 82 => x"0487", 83 => x"ff6d", 84 => x"fcf7", 85 => x"05cb", 86 => x"fda6", 87 => x"045f", 88 => x"f9ca", 89 => x"0284", 90 => x"fc98", 91 => x"015d", 92 => x"01a2", 93 => x"0149", 94 => x"ff64", 95 => x"ffb5", 96 => x"0331", 97 => x"0449", 98 => x"025b", 99 => x"0262", 100 => x"052a", 101 => x"fafb", 102 => x"fa47", 103 => x"0180", 104 => x"fb41", 105 => x"ff78", 106 => x"04c2", 107 => x"fac9", 108 => x"fc96", 109 => x"00dc", 110 => x"fb5d", 111 => x"f985", 112 => x"fb5f", 113 => x"fa06", 114 => x"fb02", 115 => x"031a", 116 => x"fa1a", 117 => x"fcaa", 118 => x"fc9a", 119 => x"01de", 120 => x"ff94", 121 => x"fecc", 122 => x"03e4", 123 => x"03df", 124 => x"03be", 125 => x"fa4c", 126 => x"05f2", 127 => x"065c");
    ...
    constant fqmul_vec_length        : integer := 4;
...
begin
    ...
    s_addr_delta <=  "00100000000" when kyber_k = "010" else -- 255
                     "00110000000" when kyber_k = "011" else -- 383
                     "01000000000" when kyber_k = "100" else -- 511
                     "00100000000";
        ...
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                if(start = '1' and s_start = '0') then
                    s_busy <= '1';
                elsif(s_en_vec(16) = '1' and s_en_vec(15) = '0') then
                    s_busy <= '0';
                else
                    s_busy <= s_busy;
                end if; 
        ...
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                if(start = '1') then  
                    ...
                    s_en_vec(16 downto 1)  <= s_en_vec(15 downto 0);                    
                else
                    s_en_vec(0)            <= '0';
                end if;
            end if;
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then     
                ...
                if(start = '1') then
                    if(or_reduce(s_en_vec(13 downto 5)) = '1') then
                    ...
                    if(s_counter_fqmul = 0) then
                        ...
                        -- fqmul1
                        s_coeff0_to_fqmul1 <= s_bram_read_doa_vec(7)(15 downto 0); -- a[0]
                        s_coeff1_to_fqmul1 <= s_bram_read_dob_vec(7)(15 downto 0); -- b[0]
                        -- fqmul4                                                           
                        s_coeff0_to_fqmul4 <= s_bram_read_doa_vec(7)(31 downto 16); -- a[1] 
                        s_coeff1_to_fqmul4 <= s_bram_read_dob_vec(7)(15 downto 0); -- b[0]  
                        
                        -- fqmul2
                        s_coeff0_to_fqmul2 <= coeff_from_fqmul2; -- r[0]
                        if(s_counter = 0 or s_counter = 2) then
                        ...
                        -- fqmul5
                        s_coeff0_to_fqmul5 <= s_bram_read_doa_vec(3)(15 downto 0); -- a[0]
                        s_coeff1_to_fqmul5 <= s_bram_read_dob_vec(3)(31 downto 16); -- b[1]                     
                        ...
                        -- fqmul3
                        s_coeff0_to_fqmul3 <= s_bram_read_doa_vec(3)(15 downto 0); -- a[0]
                        s_coeff1_to_fqmul3 <= s_bram_read_dob_vec(3)(31 downto 16); -- b[1]                        
                        ...
                        -- fqmul2
                        s_coeff0_to_fqmul2 <= s_bram_read_doa_vec(7)(15 downto 0); -- a[0]
                        s_coeff1_to_fqmul2 <= s_bram_read_dob_vec(7)(15 downto 0); -- b[0]
                        -- fqmul5                                                           
                        s_coeff0_to_fqmul5 <= s_bram_read_doa_vec(7)(31 downto 16); -- a[1] 
                        s_coeff1_to_fqmul5 <= s_bram_read_dob_vec(7)(15 downto 0); -- b[0]  
                    else
                        -- fqmul0
                        s_coeff0_to_fqmul0 <= s_bram_read_doa_vec(7)(15 downto 0); -- a[0]
                        s_coeff1_to_fqmul0 <= s_bram_read_dob_vec(7)(15 downto 0); -- b[0]
                        -- fqmul3                                                           
                        s_coeff0_to_fqmul3 <= s_bram_read_doa_vec(7)(31 downto 16); -- a[1] 
                        s_coeff1_to_fqmul3 <= s_bram_read_dob_vec(7)(15 downto 0); -- b[0]  
                        ...
                        -- fqmul4
                        s_coeff0_to_fqmul4 <= s_bram_read_doa_vec(3)(15 downto 0); -- a[0]
                        s_coeff1_to_fqmul4 <= s_bram_read_dob_vec(3)(31 downto 16); -- b[1]
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then     
                ...
                s_bram_write_wea <= s_en_vec(14);
                s_bram_write_ena <= s_en_vec(14);
                
                -- half words
                if(s_bram_write_wea = '1') then
                    s_bram_write_addra <= std_logic_vector(unsigned(s_bram_write_addra) + 1);
                else
                    s_bram_write_addra   <= (others => '0');
                end if;
                if(s_counter_fqmul = 0) then
                    s_bram_write_dia_low  <= std_logic_vector(signed(coeff_from_fqmul0) + signed(s_fqmul0_vec(3)));
                    s_bram_write_dia_high <= std_logic_vector(signed(coeff_from_fqmul3) + signed(s_fqmul3_vec(3)));
                elsif(s_counter_fqmul = 1) then 
                    s_bram_write_dia_low  <= std_logic_vector(signed(coeff_from_fqmul1) + signed(s_fqmul1_vec(3)));
                    s_bram_write_dia_high <= std_logic_vector(signed(coeff_from_fqmul4) + signed(s_fqmul4_vec(3)));
                else
                    s_bram_write_dia_low  <= std_logic_vector(signed(coeff_from_fqmul2) + signed(s_fqmul2_vec(3)));
                    s_bram_write_dia_high <= std_logic_vector(signed(coeff_from_fqmul5) + signed(s_fqmul5_vec(3)));                    
                end if; 
    ...
    bram_read_addrb      <= s_bram_read_addra or s_addr_delta;
