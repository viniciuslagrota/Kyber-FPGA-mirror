...
entity bram_port_selector_v1_0 is
	...
	port (	
        ...
        slave6_addr  : in std_logic_vector(ADDR_WIDTH-1 downto 0); 
        slave6_di    : in std_logic_vector(DATA_WIDTH-1 downto 0); 
        slave6_do    : out std_logic_vector(DATA_WIDTH-1 downto 0)  
...
begin
    ...
    master_en         <= slave0_en or slave1_en or slave2_en or slave3_en or slave4_en or slave5_en or slave6_en;
    master_we         <= slave0_we when slave0_en = '1' else
                         ...
                         slave6_we when slave6_en = '1' else 
                         '0';
    master_addr       <= slave0_addr when slave0_en = '1' else 
                         ...
                         slave6_addr when slave6_en = '1' else 
                         (others => '0');  
    master_di         <= slave0_di when slave0_en = '1' else
                         ...
                         slave6_di when slave6_en = '1' else                     
                         (others => '0'); 
    slave0_do         <= master_do;           
    slave1_do         <= master_do; 
    slave2_do         <= master_do; 
