...
architecture arch_imp of poly_tomont_v1_0 is
    ...
    -- LOCAL
    signal s_en_vec_mont    : std_logic_vector(7 downto 0);
        ...
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                if(start = '1' and s_start = '0') then
                    s_busy <= '1';
                elsif(s_en_vec_mont(5) = '1' and s_en_vec_mont(4) = '0') then
                    s_busy <= '0';
                else
                    s_busy <= s_busy;
                end if; 
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                s_en_vec_mont(7 downto 1)   <= s_en_vec_mont(6 downto 0);
            end if;
        end if;
    end process; 
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                if(start = '1') then
                    s_bram_enb   <= s_en_vec_mont(3);
                    s_bram_web   <= s_en_vec_mont(3);
