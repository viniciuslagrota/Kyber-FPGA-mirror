...
architecture arch_imp of polyvec_ntt_v1_0 is
    ...
    signal s_coeff_from_fqmul0              : std_logic_vector(15 downto 0); 
    signal s_coeff_from_fqmul1              : std_logic_vector(15 downto 0);   
    ...
    constant kyber_k_max                    : integer := 20;
    ...
    -- INDEXERS NTT
    constant index_ntt_max                  : integer := 14;
    ...
    -- LOCAL
    constant en_vec_max                     : integer := 12;
    ...
    constant register_len                   : integer  := 14;
    ...
    constant barrett_len                    : integer := 3;
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                s_coeff_from_fqmul0  <= (others => '0');
                s_coeff_from_fqmul1  <= (others => '0');
            else
                s_valid_from_fqmul0  <= valid_from_fqmul0;
                s_valid_from_fqmul1  <= valid_from_fqmul1; 
                s_coeff_from_fqmul0  <= coeff_from_fqmul0;
                s_coeff_from_fqmul1  <= coeff_from_fqmul1; 
            end if;
        end if;
    end process;  
...
--                s_bram0_addrb <= std_logic_vector(unsigned(s_last_addra) + 1);
                ...
                if(s_start = '1') then                       
                    ...
                    if(s_kyber_k_it(13) < unsigned(s_kyber_k)) then
                        if(s_en_vec(12) = '1') then
                            s_bram0_wea <= '1';  
                            s_bram0_addra <= "0" & std_logic_vector(s_kyber_k_it(14)) & std_logic_vector(s_j(13) + s_length(13));
                            s_bram0_dia   <= std_logic_vector(unsigned(s_bram0_dob_vec(9)(31 downto 16)) - unsigned(s_coeff_from_fqmul1))
                                          & std_logic_vector(unsigned(s_bram0_dob_vec(9)(15 downto 0)) - unsigned(s_coeff_from_fqmul0));
                        else
                            s_bram0_wea <= '0';                                    
                ...
                if(s_start = '1') then                       
                    ...
                    if(s_kyber_k_it(13) < unsigned(s_kyber_k)) then
                        if(s_en_vec(12) = '1') then
                            s_bram0_web <= '1';  
                            s_bram0_addrb <= "0" & std_logic_vector(s_kyber_k_it(14)) & std_logic_vector(s_j(13));
                            s_bram0_dib   <= std_logic_vector(unsigned(s_bram0_dob_vec(9)(31 downto 16)) + unsigned(s_coeff_from_fqmul1))
                                          & std_logic_vector(unsigned(s_bram0_dob_vec(9)(15 downto 0)) + unsigned(s_coeff_from_fqmul0));
                        else
                            s_bram0_web   <= '0';         
                        ...
                        s_bram0_web <= s_valid0_from_barrett;
                        s_bram0_dib   <= s_data1_from_barrett & s_data0_from_barrett;
...
--                if((s_kyber_k_it(9) = unsigned(s_kyber_k)) and (s_kyber_k_it(10) = unsigned(s_kyber_k) - 1)) then   
                ...
                s_data0_to_barrett <= s_bram0_doa(15 downto 0);
                s_data1_to_barrett <= s_bram0_doa(31 downto 16);
            end if;
        end if;
    end process;    
