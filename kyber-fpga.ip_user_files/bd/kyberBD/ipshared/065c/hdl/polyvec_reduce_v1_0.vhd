...
architecture arch_imp of polyvec_reduce_v1_0 is
    ...
    -- LOCAL
    signal s_en_vec_barrett     : std_logic_vector(7 downto 0);
    signal s_last_addr          : std_logic_vector(ADDR_WIDTH-1 downto 0);

begin
    
    s_last_addr <=  "00011111111" when kyber_k = "010" else -- 255
                    "00111111111" when kyber_k = "011" else -- 511
                    "01011111111" when kyber_k = "100" else -- 767
                    "00011111111";
        ...
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                if(start = '1' and s_start = '0') then
                    s_busy <= '1';
                elsif(s_en_vec_barrett(5) = '1' and s_en_vec_barrett(4) = '0') then
                    s_busy <= '0';
                else
                    s_busy <= s_busy;
                end if; 
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                s_en_vec_barrett(7 downto 1)   <= s_en_vec_barrett(6 downto 0);
            end if;
        end if;
    end process;   
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                if(start = '1') then
                    s_bram_enb   <= s_en_vec_barrett(3);
                    s_bram_web   <= s_en_vec_barrett(3);
