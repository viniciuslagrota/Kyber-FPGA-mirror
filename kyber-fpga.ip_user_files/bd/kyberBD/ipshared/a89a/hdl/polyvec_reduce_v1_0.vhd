...
begin
    ...
    s_valid_in_lower_barrett    <= valid_in_lower_barrett;
    s_valid_in_upper_barrett    <= valid_in_upper_barrett;
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                s_valid_vec(0)                  <= s_valid_in_lower_barrett; -- or s_valid_in_upper_barrett
    ...
    begin
        if(rising_edge(clk)) then
            if aresetn = '0' then
                ...
                if(start = '1') then
                    s_en_lower_barrett          <= s_busy;
                    s_en_upper_barrett          <= s_busy;
                    s_valid_out_lower_barrett   <= s_en_vec_barrett(1);
                    s_valid_out_upper_barrett   <= s_en_vec_barrett(1);
...
--                    s_bram_web   <= s_en_vec_barrett(7);
                    s_bram_enb   <= s_valid_in_lower_barrett;
                    s_bram_web   <= s_valid_in_lower_barrett;
    ...
    valid_out_lower_barrett <= s_valid_out_lower_barrett;
    do_lower_barrett        <= s_do_lower_barrett;     
    en_upper_barrett        <= s_en_upper_barrett;
    valid_out_upper_barrett <= s_valid_out_upper_barrett;
